            ...
            try:
                ...
                if 'Not implemented on the CPU' in str(
                    ...
                    e) or 'CUDA tensor' in str(e) or 'is_cuda()' in str(e) or "Cannot find backend for cpu" in str(e)\
                        or 'Expected a cuda device, but got: cpu' in str(e) or 'expected scalar type Float but found Half' in str(e):
                    self.model.to(torch.device('cuda'))
                    m.stride = torch.tensor([s / x.shape[-2] for x in
                                             _forward(torch.zeros(1, ch, s, s).to(torch.device('cuda')))])  # forward
                else:
                    raise e
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m is C3k2:  # for M/L/X sizes
                ...
                if scale in "mlx":
                    args[3] = True
            if m is RMELAN:  # for M/L/X sizes
                legacy = False
            # if m is DEPMD:  # for M/L/X sizes
            #     legacy = False
        elif m is AIFI:
            args = [ch[f], *args]
            ...
            c2 = sum(ch[x] for x in f)
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect, ImagePoolingAttn}:
